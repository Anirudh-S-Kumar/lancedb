...
//! Across processes the commit serializes them: every refresh carries an
//! inserted-rows filter holding a shared token (plus the provenance ids it
//! materialized), so any two refreshes of one view conflict and only one
//! lands. The loser reports a retryable conflict and replans against the
//! generation that won.
...
/// An active write spec and retained rows both disqualify. Unset drops the
/// MemWAL index, so the shard directories on storage are the durable
/// evidence of retained rows.
